...
function repairMojibake(value: string) {
  if (!/[ÃÂâ€ðŸ]/.test(value)) return value;
...
function getSafeCheckoutDescription(title: string) {
  const description = repairMojibake(title)
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[“”]/g, '"')
    .replace(/[‘’]/g, "'")
      ...
      return NextResponse.json(
        {
          error: {
            code: "INVALID_ITEMS",
            message: "Um ou mais presentes do carrinho nao estao disponiveis.",
          },
        },
        { status: 400 },
      );
      ...
      return NextResponse.json(
        {
          error: {
            ...
            message: "Dados do checkout invalidos.",
